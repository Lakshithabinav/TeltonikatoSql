...
public class ProductionService {
    ...
    private void aggregateHourlyProduction(LocalDateTime hour) {
        ...
        hourlyProduction.setFirstValue(totalFirstValue);
    ...
    private void aggregateDailyProduction(LocalDate day) {
        List<HourlyProduction> hourlyData = hourlyProductionRepository.findByTimestampBetween(day.atStartOfDay(), day.plusDays(1).atStartOfDay());

        int totalFirstValue = hourlyData.stream().mapToInt(HourlyProduction::getFirstValue).sum();
